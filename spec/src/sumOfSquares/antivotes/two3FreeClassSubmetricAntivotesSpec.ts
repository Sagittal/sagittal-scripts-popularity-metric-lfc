import {BASE_2, Grade, log, Pev, Parameter, Power, Two3FreeClass} from "@sagittal/general"
...
describe("compute23FreeClassSubmetricAntivotes", (): void => {
    it("splits the 2,3-free class into numerator and denominator, computes their submetric antivotes separately, then adjusts the denominator by k", (): void => {
        ...
        expect(actual).toBe(
            computeSubmetricAntivotes([0, 0, 0, 0, 1] as Pev<{rational: true}>, submetric) as
                Grade<LfcUnpopularityEstimate> +
            kAsCoefficient *
            computeSubmetricAntivotes([0, 0, 0, 1] as Pev<{rational: true}>, submetric) as
                Grade<LfcUnpopularityEstimate>,
        )
    })
    ...
    it("another example", (): void => {
        const kAsCoefficient = 0.46 as Parameter
        const two3FreeClass = {pev: [0, 0, 2, 0, -1]} as Two3FreeClass // {25/11}₂,₃
        const submetric = {kAsCoefficient, sum: true}

        const actual = compute23FreeClassSubmetricAntivotes(two3FreeClass, submetric)

        expect(actual).toBe(
            computeSubmetricAntivotes([0, 0, 2] as Pev<{rational: true}>, submetric) as
                Grade<LfcUnpopularityEstimate> +
            kAsCoefficient *
            computeSubmetricAntivotes([0, 0, 0, 0, 1] as Pev<{rational: true}>, submetric) as
                Grade<LfcUnpopularityEstimate>,
    ...
    it("defaults k and j to 1", (): void => {
        ...
        expect(actual).toBe(
            computeSubmetricAntivotes([0, 0, 1, -1] as Pev<{rational: true}>, submetric),
        )
    })
    ...
    it("supports deciding the numinator and diminuator by which is the greater of the two", (): void => {
        ...
        expect(actual).toBe(
            computeSubmetricAntivotes([0, 0, 0, 0, 1] as Pev<{rational: true}>, submetric) as
                Grade<LfcUnpopularityEstimate> +
            kAsCoefficient * computeSubmetricAntivotes([0, 0, 2] as Pev<{rational: true}>, submetric) as
                Grade<LfcUnpopularityEstimate>,
        )
    })
    ...
    it("works when k = 0 (and j = 1) therefore it only looks at the numerator", (): void => {
        ...
        expect(actual).toBe(
            computeSubmetricAntivotes([0, 0, 1] as Pev<{rational: true}>, submetric),
        )
    })
    ...
    it("works when j = 0 (and k = 1) therefore it only looks at the denominator", (): void => {
        ...
        expect(actual).toBe(
            computeSubmetricAntivotes([0, 0, 0, 1] as Pev<{rational: true}>, submetric),
        )
    })
    ...
    it("works when k is a logarithm base", (): void => {
        ...
        expect(actual).toBe(
            computeSubmetricAntivotes([0, 0, 1] as Pev<{rational: true}>, submetric) as
                Grade<LfcUnpopularityEstimate> +
            log(
                computeSubmetricAntivotes([0, 0, 0, 1] as Pev<{rational: true}>, submetric) as number as Power,
                BASE_2,
    ...
    it("works when k is a power exponent", (): void => {
        ...
        expect(actual).toBe(
            computeSubmetricAntivotes([0, 0, 1] as Pev<{rational: true}>, submetric) as
                Grade<LfcUnpopularityEstimate> +
            computeSubmetricAntivotes([0, 0, 0, 1] as Pev<{rational: true}>, submetric) ** 2 as
                Grade<LfcUnpopularityEstimate>,
        )
    })
    ...
    it("works when k is a power base", (): void => {
        ...
        expect(actual).toBe(
            computeSubmetricAntivotes([0, 0, 1] as Pev<{rational: true}>, submetric) as
                Grade<LfcUnpopularityEstimate> +
            2 ** computeSubmetricAntivotes([0, 0, 0, 1] as Pev<{rational: true}>, submetric) as
                Grade<LfcUnpopularityEstimate>,
        )
    })
    ...
    it("works when j is a power exponent", (): void => {
        ...
        expect(actual).toBe(
            computeSubmetricAntivotes([0, 0, 1] as Pev<{rational: true}>, submetric) ** 2 as
                Grade<LfcUnpopularityEstimate> +
            computeSubmetricAntivotes([0, 0, 0, 1] as Pev<{rational: true}>, submetric) as
                Grade<LfcUnpopularityEstimate>,
        )
    ...
    it("works when j is a power base", (): void => {
        ...
        expect(actual).toBe(
            2 ** computeSubmetricAntivotes([0, 0, 1] as Pev<{rational: true}>, submetric) as
                Grade<LfcUnpopularityEstimate> +
            computeSubmetricAntivotes([0, 0, 0, 1] as Pev<{rational: true}>, submetric) as
                Grade<LfcUnpopularityEstimate>,
        )
