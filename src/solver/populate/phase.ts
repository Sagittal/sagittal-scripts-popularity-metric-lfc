...
const computeChunkCombinations = (
    ...
    const chunkCountForParameters: Count<Chunk<PopularityParameterId>> = subtract(
        chunkCount,
        chunkCountForSubmetrics,
    ) as Count<Chunk<PopularityParameterId>>
    ...
    if (memoizedSubmetricChunkCombinations[chunkCountForSubmetrics]) {
        ...
        const submetricChunks = popularityMetricLfcScriptGroupSettings.noMoot
            ? NO_MOOT_SUBMETRIC_CHUNKS
            : SUBMETRIC_CHUNKS
        submetricChunkCombinations = computeCombinations(submetricChunks, chunkCountForSubmetrics, {
            withRepeatedElements: true,
        })
        memoizedSubmetricChunkCombinations[chunkCountForSubmetrics] = submetricChunkCombinations
        saveLog(
            `submetric combinations (with repetitions) computed: ${submetricChunkCombinations.length}; formula is ((${chunkCountForSubmetrics}+${submetricChunks.length}-1)!)/((${chunkCountForSubmetrics}!)((${submetricChunks.length}-1)!)) where ${submetricChunks.length} is the total of possible existing chunks and ${chunkCountForSubmetrics} is the count we are choosing at a time`,
            LogTarget.SETUP,
        )
    }
    ...
    if (memoizedParameterChunkCombinations[chunkCountForParameters]) {
        ...
        const parameterChunks = popularityMetricLfcScriptGroupSettings.noMoot
            ? NO_MOOT_PARAMETER_CHUNKS
            : PARAMETER_CHUNKS
        parameterChunkCombinations = computeCombinations(PARAMETER_CHUNKS, chunkCountForParameters, {
            withRepeatedElements: true,
        })
        memoizedParameterChunkCombinations[chunkCountForParameters] = parameterChunkCombinations
        saveLog(
            `parameter combinations (with repetitions) computed: ${parameterChunkCombinations.length}; formula is ((${chunkCountForParameters}+${parameterChunks.length}-1)!)/((${chunkCountForParameters}!)((${parameterChunks.length}-1)!)) where ${parameterChunks.length} is the total of possible existing chunks and ${chunkCountForParameters} is the count we are choosing at a time`,
            LogTarget.SETUP,
        )
    }
    ...
    saveLog(
        `we find ${exampleDistributions.length} distributions of ${parameterChunkCombinations[0].length} parameter chunks across ${submetricChunkCombinations[0].length} bins (assignments to each of a combination of submetrics, plus an extra bin for parameters which will get applied to every submetric), which is how many more scopes should be contributed per each of the ${parameterChunkCombinations.length} parameter chunk combinations in this phase, and that times the ${submetricChunkCombinations.length} submetric chunk combinations in this phase, so expect ${exampleDistributions.length} * ${parameterChunkCombinations.length} * ${submetricChunkCombinations.length} = ${exampleDistributions.length * parameterChunkCombinations.length * submetricChunkCombinations.length} new scopes from this phase, so we should end with a total of ${solverStatus.populatedScopeCount + exampleDistributions.length * parameterChunkCombinations.length * submetricChunkCombinations.length}`,
        LogTarget.SETUP,
    )
...
const populateScopesPhase = async (
    ...
    const { submetricChunkCombinations, parameterChunkCombinations } = computeChunkCombinations(
        chunkCount,
        chunkCountForSubmetrics,
    )
    ...
    for (const [
    ...
    ] of submetricChunkCombinations.entries()) {
        await populateScopesForSubmetricChunkCombination(submetricChunkCombination, {
            ...
            submetricChunkCombinationIndex: submetricChunkCombinationIndex as Index<
    ...
    saveLog(
        `finished phase ${1 + chunkCount - chunkCountForSubmetrics}/${chunkCount} of scope population ${formatSearchedAndPopulated()}`,
        LogTarget.SETUP,
    )
}
...
export { populateScopesPhase, populateScopesPhaseSync }
