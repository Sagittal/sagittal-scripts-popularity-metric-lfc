import { Filename, LogTarget, Parameter, saveLog, stringify, Window } from "@sagittal/general"
...
applySharedPopularityMetricLfcScriptSetup({ logDir: "perfectMetricFromScope" as Filename })
...
if (popularityMetricLfcScriptGroupSettings.sync) {
    ...
    void recursiveSearchScopeAndMaybeUpdateBestMetric(scope, { onlyBetterThanSopfgtt: false }).then(
        (): void => {
            saveLog(`\nbest metric: ${stringify(Object.fromEntries(bestMetrics))}`, LogTarget.FINAL)
        },
    )
