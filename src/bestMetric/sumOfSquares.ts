...
const computeSumOfSquaresAndMaybeUpdateBestMetricSync = (
    ...
    try {
        ...
        if (e instanceof Error) saveLog(`error when computing sum of squares: ${e.message}`, LogTarget.ERROR)
    }
    setSumOfSquaresAtSamplePoint(sumOfSquares, sumsOfSquares, samplePoint)

    const bestMetric = bestMetrics.get(metricName)
    if (shouldUpdateBestMetric({ sumOfSquares, bestMetric, onlyBetterThanSopfgtt })) {
        ...
        saveLog(`${indentation}new best metric: ${stringify(bestMetric)}` as Io, LogTarget.RESULT)
    }
}
...
export { computeSumOfSquaresAndMaybeUpdateBestMetric, computeSumOfSquaresAndMaybeUpdateBestMetricSync }
