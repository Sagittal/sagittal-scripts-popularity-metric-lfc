...
const computeSpreadDynamicParameters = (scope: Scope): Maybe<PopularityParameterId[]> => {
    ...
    const spreadParameters: PopularityParameterId[] = Object.keys(
        allBinsSubmetricScope,
    ) as PopularityParameterId[]

    const spreadDynamicParameters = spreadParameters.filter(
        (spreadParameter: PopularityParameterId): boolean => {
            return PARAMETER_DYNAMISMS[spreadParameter]
        },
    )

    return spreadDynamicParameters.length ? spreadDynamicParameters : undefined
}

export { computeSpreadDynamicParameters }
