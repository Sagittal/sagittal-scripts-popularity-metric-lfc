...
const computeDynamicParameterScope = (options: DynamicParameterScopeOptions): DynamicParameterScope => {
    ...
    if (definedOptionCount !== 2) {
        const providedOptions = Object.entries(options)
            .map(([k, v]: [string, number]): string => `${k} ${v}`)
            .join(", ")
        throw new Error(
            `Exactly 2 options should be provided from min, max, center, and window in order to compute a dynamic parameter scope; ${definedOptionCount} provided (${providedOptions})`,
        )
    }
    ...
    const ed = computeEqualDivision(window as Window<{ of: Parameter }>)

    return { center, window, ed }
}

export { computeDynamicParameterScope }
